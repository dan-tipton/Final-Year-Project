...
class BPASSAnalysis():
    ...
    def subhaloData(self, path):
        ...
        supernova_rate = all_results_df["ccSNRate"] / (all_results_df["Age_Myr"] * 1e6)
        supernova_rate_per_solar_mass = supernova_rate / all_results_df["Mass"]
        ...
        # - divide by the halo volume
        # - Possibly Correct - multiple by number of galaxies/box volume

        # halo volume calculated in Gpc^3 from TNG gas data
        # sum the total snr per solar mass for the subhalo 
        subhalo_snr = sum(supernova_rate_per_solar_mass)
